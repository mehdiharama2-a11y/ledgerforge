...
public class PaymentController {
  ...
  PaymentView capture(@AuthenticationPrincipal Jwt jwt, @RequestHeader("Idempotency-Key") @Size(min=1,max=128) String key, @Valid @RequestBody CaptureRequest request) {
  ...
  @PreAuthorize("hasAnyRole('ADMIN','OPERATOR')")
  RefundView refund(@AuthenticationPrincipal Jwt jwt, @RequestHeader("Idempotency-Key") @Size(min=1,max=128) String key, @PathVariable UUID paymentId, @Valid @RequestBody RefundRequest request) {
  ...
  record CaptureRequest(@NotNull @DecimalMin("0.01") BigDecimal amount,@NotBlank @Pattern(regexp="^[A-Z]{3}$") String currency) {}
